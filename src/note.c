...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
    ...
    if (ch->cabal && is_name(cabal_table[ch->cabal].name,pnote->to_list))
	return TRUE;
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	if (is_name(argument,"all") && !(IS_IMMORTAL(ch)))
	{
		send_to_char("Only Immortals can send notes to all.\n\r",ch);
		return;
	}
    ...
    if ( !str_prefix( arg, "post" ) || !str_prefix(arg, "send"))
		...
		/*if (is_affected(ch,gsn_cloak_form));
		{
		    send_to_char("You must be uncloaked to post a note.\n\r", ch);
		    return;
		}*/
		...
		// A little message so you know you've gotten a new note *Ceial*

		if ( is_note_to( d->character, ch->pnote ) && ( d->character != ch ) )
		{
			if ( (IS_IMMORTAL(ch)) && (ch->invis_level > d->character->level ) )
			{
				;
			}
			else if (!can_see(d->character, ch))
			{
				send_to_char("A messenger hands you a scroll from an unseen being.\n\r", d->character );
			}
			else if ( is_note_to( d->character, ch->pnote ) && ( d->character == ch ) )
			{
				send_to_char( "Your messenger has delievered your scroll.\n\r", ch );
			}
			else
			{
				sprintf( buf, "A messenger hands you a scroll from %s.\n\r", ch->name );
