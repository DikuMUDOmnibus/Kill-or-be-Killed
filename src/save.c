...
fprintf(fp,"Lev %2d Trust %2d  %s%s\n",
	...
	for (search = char_list; search != NULL; search = search->next)
                if (IS_NPC(search) && (search->master == ch) && IS_SET(search->act,ACT_UNDEAD)
...
void fwrite_charmie(CHAR_DATA *ch, FILE *fp)
{
    ...
    fprintf(fp, "%s~\n",	ch->long_descr);
    fprintf(fp, "%d %d %d \n",	ch->level, ch->max_hit, ch->hit);
    fprintf(fp, "%d %d %d %d\n",ch->alignment, ch->damage[DICE_NUMBER], ch->damage[DICE_TYPE], ch->damroll);
}
...
void fread_charmie(CHAR_DATA *ch, FILE *fp)
{
	...
	if (get_mob_index(vnum) == NULL)
	{
            ...
            charmed = create_mobile(get_mob_index(MOB_VNUM_FIDO));
        }
        else
            charmed = create_mobile(get_mob_index(vnum));
	free_string(charmed->short_descr);
	free_string(charmed->long_descr);
	free_string(charmed->name);
	charmed->name		= str_dup(fread_string(fp));
	charmed->short_descr 	= str_dup(fread_string(fp));
	charmed->long_descr	= str_dup(fread_string(fp));
	...
	char_to_room(charmed,ch->in_room);
...
/* kills and killed scores */
	...
	{
	    if(ch->pcdata->forms[pos])
		fprintf(fp, "Form %s~ %d\n",capitalize(form_table[pos].name),ch->pcdata->forms[pos]);
	}
	...
	fprintf( fp, "Affc '%s' %3d %3d %3d %3d %3d %10d %3d\n",
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    if (found && ch->version != 7)
    {
		if (ch->cabal == CABAL_OUTLAW)
		{
			...
			ch->pcdata->learned[skill_lookup("garble")] = 0;
			ch->pcdata->learned[skill_lookup("confuse")] = 0;
			ch->pcdata->learned[skill_lookup("chaos blade")] = 75;
			ch->pcdata->learned[skill_lookup("randomizer")] = 75;
			ch->pcdata->learned[skill_lookup("pandemonium")] = 0;
...
{
    char buf[MAX_STRING_LENGTH];
    char cloakbuf[MAX_STRING_LENGTH];
    char *word; //, bugMsg[80];
    bool fMatch;
    int count = 0;
	...
	case 'E':
		...
		if(is_affected(ch,gsn_cloak_form)) {
			...
			sprintf(cloakbuf, "cloaked rider %s",ch->name);
			ch->name=str_dup(cloakbuf);
			ch->name=str_dup("cloaked rider");
		}
		return;
            }
