...
"wield", "hold", "dual_wield","float", "trinal_wield","brand","bound",NULL
};
...
void do_induct( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_prefix(arg2,"empire") && !IS_IMMORTAL(ch))
    {
    ...
    if (cabal == CABAL_COVEN)
    {
	victim->pcdata->learned[skill_lookup("rift")]	  	        = 0;
	victim->pcdata->learned[skill_lookup("mental drain")] 		= 0;
    victim->pcdata->learned[skill_lookup("transcendence")]		= 0;
    victim->pcdata->learned[skill_lookup("shifting colors")]    = 0;
    victim->pcdata->learned[skill_lookup("spellcraft II")]		= 0;
    }
    else if (cabal == CABAL_ACADIAN)
    {
	victim->pcdata->learned[gsn_eye_of_the_predator] = 0;
	victim->pcdata->learned[gsn_shadowgate] = 0;
        victim->pcdata->learned[skill_lookup("slave of purgatory")] = 0;
        victim->pcdata->learned[skill_lookup("shroud")] = 0;
        victim->pcdata->learned[skill_lookup("eyes of intrigue")] = 0;
	victim->pcdata->learned[skill_lookup("ritual of draal'ukul")] = 0;
		victim->pcdata->learned[skill_lookup("mantle of maehslin")] = 0;
	victim->pcdata->learned[skill_lookup("darkforge")] = 0;

    }
    else if (cabal == CABAL_BRIAR) /* Just for now */
    {
        victim->pcdata->learned[skill_lookup("chameleon")] = 0;
        victim->pcdata->learned[skill_lookup("insectswarm")] = 0;
        victim->pcdata->learned[skill_lookup("beast call")] = 0;
        victim->pcdata->learned[skill_lookup("spiderhands")] = 0;
        victim->pcdata->learned[skill_lookup("lifeline")] = 0;
	victim->pcdata->learned[skill_lookup("dream blossom")] = 0;
    }
    else if (cabal == CABAL_PROTECTOR)
    {
	victim->pcdata->learned[skill_lookup("lightwalk")] = 0;
	victim->pcdata->learned[skill_lookup("dragonplate")] = 0;
	victim->pcdata->learned[skill_lookup("dragonweapon")] = 0;
	victim->pcdata->learned[skill_lookup("blademaster")] = 0;
	victim->pcdata->learned[skill_lookup("truestrike")] = 0;
    }
    else if (cabal == CABAL_RAVAGER)
    {
	victim->pcdata->learned[skill_lookup("resist damage")] = 0;
	victim->pcdata->learned[skill_lookup("trek")]          = 0;
	victim->pcdata->learned[skill_lookup("claymore")]      = 0;
	victim->pcdata->learned[skill_lookup("battlepaint")]   = 0;
	victim->pcdata->learned[skill_lookup("firstaid")]      = 0;
	victim->pcdata->learned[skill_lookup("swing")] 		   = 0;
    }
    else /*if (cabal == CABAL_VIGILANTE)*/
    {
	...
	victim->pcdata->learned[gsn_shackles] = 0;
    victim->pcdata->learned[gsn_divine_aura] = 0;
    victim->pcdata->learned[skill_lookup("stalker")] = 0;
    }
            victim->cabal = 0;
            sprintf(buf,"AUTO: Uninducted from %s by %s.\n\r",cabal ? capitalize(cabal_table[cabal].name) : "-",ch->name);
    ...
    sprintf(buf,"%s has been inducted into %s.\n\r",
        victim->name, capitalize(cabal_table[cabal].name));
    send_to_char(buf,ch);
    sprintf(buf,"You have been inducted into %s.\n\r",
        capitalize(cabal_table[cabal].name));
    send_to_char(buf, victim);
    group_remove(victim,cabal_table[victim->cabal].name);
    victim->cabal = cabal;

    sprintf(buf,"AUTO: Inducted into %s by %s.\n\r",capitalize(cabal_table[cabal].name),ch->name);
    if (!IS_IMMORTAL(victim))
                add_history(NULL,victim,buf);

    group_add(victim,cabal_table[cabal].name,FALSE);
    if (cabal == CABAL_ACADIAN)
    {
        victim->pcdata->learned[gsn_eye_of_the_predator] = 75;
        victim->pcdata->learned[gsn_shadowgate] = 75;
        victim->pcdata->learned[skill_lookup("slave of purgatory")] = 75;
        victim->pcdata->learned[skill_lookup("shroud")] = 75;
        victim->pcdata->learned[skill_lookup("eyes of intrigue")] = 75;
		victim->pcdata->learned[skill_lookup("ritual of draal'ukul")] = 75;
        victim->pcdata->learned[skill_lookup("mantle of maehslin")] = 75;
        victim->pcdata->learned[skill_lookup("darkforge")] = 75;
    }
    else if (cabal == CABAL_BRIAR)
    {
        victim->pcdata->learned[skill_lookup("chameleon")] = 75;
        victim->pcdata->learned[skill_lookup("insectswarm")] = 75;
        victim->pcdata->learned[skill_lookup("beast call")] = 75;
        victim->pcdata->learned[skill_lookup("spiderhands")] = 75;
        victim->pcdata->learned[skill_lookup("lifeline")] = 75;
	victim->pcdata->learned[skill_lookup("dream blossom")] = 75;
    }
    else if (cabal == CABAL_PROTECTOR)
    {
	victim->pcdata->learned[skill_lookup("lightwalk")] = 75;
	victim->pcdata->learned[skill_lookup("dragonplate")] = 75;
	victim->pcdata->learned[skill_lookup("dragonweapon")] = 75;
	victim->pcdata->learned[skill_lookup("blademaster")] = 75;
	victim->pcdata->learned[skill_lookup("truestrike")] = 75;
	victim->pcdata->learned[skill_lookup("gouge")] = 0;
	victim->pcdata->learned[skill_lookup("stab")] = 0;
    }
    else if (cabal == CABAL_COVEN)
    {
	victim->pcdata->learned[skill_lookup("rift")]	  	        = 75;
	victim->pcdata->learned[skill_lookup("mental drain")] 		= 75;
    victim->pcdata->learned[skill_lookup("transcendence")]		= 75;
    victim->pcdata->learned[skill_lookup("shifting colors")]    = 75;
    victim->pcdata->learned[skill_lookup("spellcraft II")]		= 75;
    }
    else if (cabal == CABAL_RAVAGER)
    {
	victim->pcdata->learned[skill_lookup("resist damage")] 	 = 75;
	victim->pcdata->learned[skill_lookup("trek")] 			 = 100;
	victim->pcdata->learned[skill_lookup("claymore")] 	 	 = 75;
	victim->pcdata->learned[skill_lookup("battlepaint")] 	 = 75;
	victim->pcdata->learned[skill_lookup("firstaid")] 	 	 = 75;
	victim->pcdata->learned[skill_lookup("swing")]			 = 75;
    }
    else /*if (cabal == CABAL_VIGILANTE)*/
    {
	...
	victim->pcdata->learned[gsn_shackles] = 100;
    victim->pcdata->learned[gsn_divine_aura] = 75;
    victim->pcdata->learned[skill_lookup("stalker")] = 75;
    }
...
void do_leaderpeek( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn < MAX_SKILL; sn++ )
	{
	    ...
	    if ( victim->level < skill_table[sn].skill_level[victim->class]
	      || victim->pcdata->learned[sn] < 95 /* skill not high enough */)
		continue;
...
/*void easy_induct( CHAR_DATA *ch, char *argument )
{
	...
	ch->pcdata->learned[skill_lookup("ritual of draal'ukul")] = 75;
        ch->pcdata->learned[skill_lookup("shield of the onyx")] = 75;
    ...
    else if (cabal == CABAL_BRIAR)
    {
        ...
        ch->pcdata->learned[skill_lookup("forest friend")] = 75;
        ch->pcdata->learned[skill_lookup("spiderhands")] = 75;
        ch->pcdata->learned[skill_lookup("lifeline")] = 75;
	ch->pcdata->learned[skill_lookup("dream blossom")] = 75;
    }
    else if (cabal == CABAL_PROTECTOR)
    {
	...
	ch->pcdata->learned[skill_lookup("life transfer")] = 75;
	ch->pcdata->learned[skill_lookup("lightshield")] = 75;
	ch->pcdata->learned[skill_lookup("gouge")] = 0;
	ch->pcdata->learned[skill_lookup("stab")] = 0;
    }
    else if (cabal == CABAL_COVEN)
    {
	ch->pcdata->learned[skill_lookup("familiar")] = 75;
	ch->pcdata->learned[skill_lookup("brew")] = 75;
	ch->pcdata->learned[skill_lookup("spiritblade")] = 75;
	ch->pcdata->learned[skill_lookup("venueport")] = 75;
	ch->pcdata->learned[skill_lookup("team spirit")] = 75;
	...
	ch->pcdata->learned[gsn_battlecry] = 75;
	ch->pcdata->learned[gsn_steel_nerves] = 75;
	ch->pcdata->learned[gsn_blitz] = 75;
	ch->pcdata->learned[gsn_bloodthirst] = 75;
    }
    else if (cabal == CABAL_VIGILANTE)
    {
 	ch->pcdata->learned[skill_lookup("judgement")] = 75;
 	ch->pcdata->learned[skill_lookup("trace")] = 75;
	ch->pcdata->learned[gsn_true_sight] = 75;
	ch->pcdata->learned[skill_lookup("hand of vengeance")] = 75;
	ch->pcdata->learned[gsn_wanted] = 100;
	ch->pcdata->learned[skill_lookup("guard call")] = 75;
	ch->pcdata->learned[skill_lookup("protectorate plate")] = 75;
        ch->pcdata->learned[skill_lookup("stalker")] = 75;
    }
    else if (cabal == CABAL_OUTLAW)
    {
	ch->pcdata->learned[skill_lookup("decoy")] = 75;
	ch->pcdata->learned[skill_lookup("badge")] = 75;
	ch->pcdata->learned[gsn_disperse] = 75;
	ch->pcdata->learned[skill_lookup("hacksaw")] = 75;
	//victim->pcdata->learned[skill_lookup("garble")] = 75;
	//victim->pcdata->learned[skill_lookup("confuse")] = 75;
	ch->pcdata->learned[skill_lookup("chaos blade")] = 75;
	ch->pcdata->learned[skill_lookup("randomizer")] = 75;
	//victim->pcdata->learned[skill_lookup("pandemonium")] = 75;
	ch->pcdata->learned[skill_lookup("call slaves")] = 75;
	ch->pcdata->learned[skill_lookup("defiance")] = 75;
	ch->pcdata->learned[skill_lookup("coerce")] = 75;
    }
    ...
    sprintf(buf,"%s has been inducted into %s.\n\r",ch->name, capitalize(cabal_table[cabal].name));
    log_string( buf );
}
*/
...
//IS_NPC(ch) ? 0 : ch->pcdata->special);
    ...
    sprintf( buf,
        "{WHit: {R%d  {WDam: {R%d  {WSaves: {R%d{x Size: %s Position: %s Wimpy: %d\n\r",
...
victim->pcdata->age_mod);
    ...
    {
	send_to_char("Extended Offense: ",ch);
	extflag=1;
	for(i=0;i<25;i++)
	{
		if(!extended_flags[i].bit)
			break;
		if(IS_SET(victim->extended_flags,extflag))
		{
			send_to_char(extended_flags[i].name,ch);
			send_to_char(" ",ch);
		}
		extflag*=2;
	}
	...
	if (!IS_NPC(victim))
	{
		sprintf(buf, "Bounty Worth: %d.\n\r",victim->pcdata->bounty );
		send_to_char( buf, ch);
	}

	if (!IS_NPC(victim))
	{
		sprintf(buf, "Bounty Kills: %d.\n\r",victim->pcdata->bkills );
		send_to_char( buf, ch);
	}
	...
	if ( (!IS_NPC(victim)) && (ch->class == 10) )
	{
		sprintf(buf, "Element: %d. \n\r",victim->pcdata->element );
		send_to_char( buf, ch );
		send_to_char("1 is Air, 2 is Earth, 3 is Ice, 4 is Fire.\n\r",ch);
	}

	if (!IS_NPC(victim))
	{
		sprintf( buf, "Worshipping: %s.\n\r", victim->pcdata->god );
		send_to_char( buf, ch );
	}
...
/* tracking stuff */
	...
	if (paf->aftype == AFT_SPELL)
		...
		sprintf( buf2, "Song");
 		...
 		sprintf( buf2, "Spell");
	sprintf( buf,
	"%s: '%s' %s%s%smodifies %s by %d for %d hours with %s-bits %s, owner %s, level %d.\n\r",
	buf2,
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf( buf, "{BHold onto your hats!{x");
	do_echo( ch, buf );
	sprintf( buf, "{BRebooting now!{x");
	do_echo( ch, buf );
    }
	...
	if (vch != NULL && d->connected == 0)
	{
		...
		if (IS_SET(vch->comm, COMM_MOUNTED))
		{
			REMOVE_BIT(vch->comm, COMM_MOUNTED);
		}
...
{
    char arg[MAX_INPUT_LENGTH];
    char arg2[MIL], arg3[MIL], arg4[MIL], buf[1000];
    bool fAll = FALSE;
    int i, form;
    CHAR_DATA *victim;
	...
	send_to_char("  set form  <name> <form/all> <value>\n\r", ch);
	return;
    }
    ...
    if(!str_prefix(arg,"form"))
    {

	argument = one_argument(argument,arg2);
	argument = one_argument(argument,arg3);
	argument = one_argument(argument,arg4);
	if(!arg2 || !arg3 || !arg4 || !str_cmp(arg2,""))
		return send_to_char("Invalid form.\n\r",ch);
	if(!(victim = get_char_world(ch,arg2)))
		return send_to_char("They aren't here.\n\r",ch);
	if(IS_NPC(victim))
		return send_to_char("You can't do that to NPCs.\n\r",ch);
	if ( !str_cmp(arg3,"all"))
		fAll = TRUE;
	if((form = form_lookup(arg3))<0 && !fAll)
		return send_to_char("That's not a valid form.\n\r",ch);
	if(!is_number(arg4))
		return send_to_char("Last argument must be numeric.\n\r",ch);
	if(!fAll)
	{
		victim->pcdata->forms[form] = atoi(arg4);
		sprintf(buf,"%s now has the %s form at %s%%.\n\r",
		victim->original_name,
		form_table[form].name,
		arg4);
		send_to_char(buf,ch);
		return;
	}
	...
	{
                victim->pcdata->forms[i] = atoi(arg4);
                sprintf(buf,"%s now has the %s form at %s%%.\n\r",
                victim->original_name,
                form_table[i].name,
                arg4);
...
hostnames.
 */
	    sprintf( buf + strlen(buf), "{B[{W%3d {G%14s {W%4d{B]{x %s@%s\n\r",
		d->descriptor,
...
void do_arena_echo(const char *txt)
{
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		if ( d->connected == CON_PLAYING )
		{
			...
			send_to_char( "\x01B[1;31mARENA>\x01B[0;37m ",d->character);
			send_to_char( buffer, d->character );
			send_to_char( "\n\r",   d->character );
		}
	}
...
void do_nocast(CHAR_DATA *ch, char *argument) //Ceials shit, DON'T TOUCH!
{ 							  //Thank you. :)
	char arg1[MAX_INPUT_LENGTH];
	char buf[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
	...
	{
		buf[0] = '\0';
	    send_to_char( "Syntax: nocast <character>\n\r",ch);
	   	return;
	}
	...
	if ( IS_SET(victim->comm, COMM_NOCAST) )
	{
	    ...
	    send_to_char( "You can now cast spells.\n\r",victim );
	    send_to_char( "They can now cast spells.\n\r", ch );
		sprintf(buf,"$N restores casting ability to %s",victim->name);
		}
	    else
	    {
	    SET_BIT(victim->comm, COMM_NOCAST);
	    send_to_char( "You no longer can cast spells.\n\r",victim );
	    send_to_char( "They no longer can cast spells.\n\r", ch );
		sprintf(buf,"$N revokes %s's ability to cast spells.",victim->name);
		}

	return;
}

void do_magicproof(CHAR_DATA *ch)
{

	if (IS_SET(ch->in_room->room_flags, ROOM_NO_MAGIC) )
	{
		send_to_char("Magic is now allowed here.\n\r",ch);
		REMOVE_BIT(ch->in_room->room_flags,ROOM_NO_MAGIC);
	}
	else
	{
		send_to_char("Magic is no longer allowed here.\n\r",ch);
		SET_BIT(ch->in_room->room_flags,ROOM_NO_MAGIC);
	}
	return;
}
...
*/
	...
	for (d = descriptor_list; d ; d = d_next)
	{
		...
		{
			if (is_affected(och,gsn_cloak_form))
			{
				affect_strip(och,gsn_cloak_form);
				write_to_descriptor (d->descriptor, "\n\rRemoving cloak affect...\n\r",0);
				free_string(och->name);
				och->name=str_dup(och->original_name);
				free_string(och->original_name);
				och->original_name=NULL;
				if(is_affected(och,gsn_eye_of_the_predator))
				{
					affect_strip(ch,gsn_eye_of_the_predator);
					write_to_descriptor (d->descriptor, "\n\rRemoving eye of the predator affect...\n\r",0);
				}
			}
	...
	execl (EXE_FILE, "kbk3", buf, "copyover", buf2, (char *) NULL);
